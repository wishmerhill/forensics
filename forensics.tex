...
\begin{document}
	...
	\section{Fonti normative}
		...
		\subsection{Come operare con dispositivi mobili (\textit{telefoni, smartphone e tablet})}
			...
			Come sempre, vale la regola di non accendere mai gli apparecchi rinvenuti spenti. Prima di imballarli, vanno ricercate e sequestrate a parte eventuali SIM card e schede di memoria contenute nel telefono, delle quali vanno annotati numeri di serie e ogni altro dato utile.
			
			Il numero IMEI del telefono (ovvero il numero identificativo univoco del terminale), solitamente riportato su un etichetta posta sulla scocca al di sotto della batteria: pertanto è necessario, dove possibile, rimuoverla per poter annotare l'IMEI: tale operazione, su un terminale rinvenuto spento, ha il grosso svantaggio che azzera i dati della memoria volatile del telefono, quali le chiamate recenti e le impostazioni di data e ora.
		...
		L'errore dei malviventi era stato quello di operare sempre in filiali della stessa catena (Mercatone Uno) usando documenti veri e validi: la direzione centrale della catena, dopo l'ennesima truffa, aveva diramato le foto dei truffatori, permettendo quindi ai commessi di riconoscere i soggetti che stavano per truffarli e chiamare i Carabinieri.
		
		Come si diceva, dunque, i colleghi del Nucleo Operativo, identificata l'auto con la quale i truffatori si stavano dileguando, riuscirono a fermarla e, nel corso di una ispezione al veicolo, trovarono un computer portatile, una stampante e un \textit{inverter} da campeggio da 12 a 220 Volt occultati in uno scatolone nel bagagliaio.
		...
		\paragraph{} Purtroppo però non fu stato possibile aprire il file in quanto era solo un \textit{link} che faceva riferimento ad una cartella contenuta in una memoria rimovibile (forse una penna USB): tuttavia questo indizio, per quanto lieve, era più che sufficiente per chiudere la perquisizione e procedere al sequestro per effettuare successivamente un'analisi forense approfondita alla ricerca di eventuali tracce di assegni contraffatti.
		...
		La risposta quindi alla domanda, posta nel titolo del paragrafo, se esista o meno la ``perquisizione informatica'' è sì: di volta in volta la PG, di concerto con l'A.G., valuta come operare su un computer. Di fatto, quello che è stato fatto nell'esempio descritto non è altro che una perquisizione, \textbf{irripetibile ma documentata}, dell'apparecchiatura informatica.
	...
	\section{L'analisi forense}
		...
		\subsection{L'analisi \textit{post mortem}}
			...
			\subsubsection{La copia forense}
				...
				Una volta effettuata la copia -- o immagine -- forense è necessario (in realtà indispensabile) procedere al calcolo dell'\textit{hash}. In realtà, 
