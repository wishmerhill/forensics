...
\begin{document}
	...
	\section{Premessa}
		...
		Se ad esempio fino a qualche anno fa la regola nel repertare un \textit{personal computer} acceso era quella di spegnerlo senza particolari cautele (staccando molto semplicemente la spina,) per poi procedere alla sigillatura delle porte e alla chiusura del plico, oggi non si opera più in questo modo, almeno non sempre: si è visto infatti che il contenuto della memoria volatile del computer può essere determinante o comunque di notevole importanza per le indagini. 
		...
		\subsection{Su questo documento}
			La trattazione di una materia come quella del \textit{Digital Forensic} non può essere esaurita in questa trentina di pagine, né lo scrivente ha la presunzione di aver scritto (o meglio, raccolto) solo verità assolute. 
	...
	\section{Fonti normative}
		...
		\subsubsection{Caso 1: computer spento}
			...
			Inoltre, sapere con certezza quali operazioni sono state svolte dalla Polizia Giudiziaria è fondamentale anche in sede dibattimentale, per evitare che di quelle eventuali azioni -- non documentate in alcun atto di indagine -- si accorgano i periti di parte con conseguente figuraccia, come insegna il caso di Garlasco.
		...
		\subsection{Come operare con dispositivi mobili (\textit{telefoni, smartphone e tablet})}
			...
			Per spegnerlo è necessario staccare la batteria\footnote{
				Ovviamente non è sempre così semplice: ormai sempre più telefoni hanno la batteria integrata e spegnerlo potrebbe essere problematico. Queste situazioni, purtroppo, vanno valutate di caso in caso.},
	...
	\section{L'analisi forense}
		...
		\subsection{L'analisi \textit{post mortem}}
			...
			\subsubsection{I software per l'analisi forense}\label{subsubsec:swForensi}
				...
				Tra i software commerciali deve essere citato quello che è riconosciuto unanimemente come lo standard industriale: \textit{EnCase Forensic} della Guidance Software. Si tratta di un software completo per l'analisi e il reporting su computer, dispositivi mobili, memorie di massa, ecc.. Dato il costo della licenza non permette di testalo e quindi chi scrive non è in grado di interloquire o di darne una descrizione più dettagliata.
			...
			\subsubsection{Hash, impronta digitale elettronica}
				...
				Come è stato detto in precedenza, la funzione di hashing è induttiva e non reversibile e la probabilità di collisioni (ovvero di due serie di dati che forniscano la stessa sequenza di bit) è estremamente improbabile sebbene non impossibile ed è inversamente proporzionale alla complessità dell'algoritmo di calcolo.
...
%		\url{http://www.marcomattiucci.it/informatica}
%			
%	\end{thebibliography}
